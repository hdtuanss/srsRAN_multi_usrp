                              ...
                              void* stream_handler)
{
  ...
  if (q != NULL) {
    ...
    q->sf_buffer[1]      = srsran_vec_cf_malloc(CELL_SEARCH_BUFFER_MAX_SAMPLES);
    q->sf_buffer_b[0]    = srsran_vec_cf_malloc(CELL_SEARCH_BUFFER_MAX_SAMPLES);
    q->sf_buffer_b[1]    = srsran_vec_cf_malloc(CELL_SEARCH_BUFFER_MAX_SAMPLES);
    q->nof_rx_antennas = 1;
...
int srsran_ue_cellsearch_scan_multi_usrp(srsran_ue_cellsearch_t*       q,
                                          srsran_ue_cellsearch_result_t found_cells[3],
                                          uint32_t*                     max_N_id_2)
{
  int      ret                = 0;
  ...
  for (uint32_t N_id_2 = 0; N_id_2 < 3; N_id_2++) {
    INFO("CELL SEARCH: Starting scan for N_id_2=%d", N_id_2);
    ret = srsran_ue_cellsearch_scan_N_id_2_multi_usrp(q, N_id_2, &found_cells[N_id_2]);
    if (ret < 0) {
      ERROR("Error searching cell");
      return ret;
    }
...
int srsran_ue_cellsearch_scan_N_id_2_multi_usrp(srsran_ue_cellsearch_t*        q,
                                                uint32_t                       N_id_2,
                                                srsran_ue_cellsearch_result_t* found_cell)
{
  int      ret                 = SRSRAN_ERROR_INVALID_INPUTS;
