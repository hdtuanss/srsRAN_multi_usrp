...
static cf_t  dummy_buffer0_a[DUMMY_BUFFER_NUM_SAMPLES];
static cf_t  dummy_buffer1_a[DUMMY_BUFFER_NUM_SAMPLES];
static cf_t  dummy_buffer0_b[DUMMY_BUFFER_NUM_SAMPLES];
static cf_t  dummy_buffer1_b[DUMMY_BUFFER_NUM_SAMPLES];
static cf_t* dummy_offset_buffer_a[SRSRAN_MAX_CHANNELS] = {dummy_buffer0_a, dummy_buffer1_a, dummy_buffer1_a, dummy_buffer1_a};
static cf_t* dummy_offset_buffer_b[SRSRAN_MAX_CHANNELS] = {dummy_buffer0_b, dummy_buffer1_b, dummy_buffer1_b, dummy_buffer1_b};
...
int srsran_ue_sync_init_multi_usrp(
    srsran_ue_sync_t* q,
    uint32_t          max_prb,
    bool              search_cell,
    int(recv_callback_multi)(void*, void*, cf_t* [SRSRAN_MAX_CHANNELS], cf_t* [SRSRAN_MAX_CHANNELS], uint32_t, srsran_timestamp_t*),
    int(recv_callback)(void*, cf_t* [SRSRAN_MAX_CHANNELS], uint32_t, srsran_timestamp_t*),
    uint32_t nof_rx_antennas,
    void*    stream_handler_a,
    void*    stream_handler_b,
    int      decimate)
{
  return srsran_ue_sync_init_multi_usrp_decim_mode(
      q, max_prb, search_cell, recv_callback_multi, recv_callback, nof_rx_antennas, stream_handler_a, stream_handler_b, 1, SYNC_MODE_PSS);
}

int srsran_ue_sync_init_multi_usrp_decim_mode(
    srsran_ue_sync_t* q,
    uint32_t          max_prb,
    bool              search_cell,
    int(recv_callback_multi)(void*, void*, cf_t* [SRSRAN_MAX_CHANNELS], cf_t* [SRSRAN_MAX_CHANNELS], uint32_t, srsran_timestamp_t*),
    ...
    srsran_ue_sync_mode_t mode)
{
  ...
  if (q != NULL && stream_handler_a != NULL&& stream_handler_b != NULL&& nof_rx_antennas <= SRSRAN_MAX_CHANNELS&&recv_callback_multi != NULL && recv_callback != NULL) {
    ret = SRSRAN_ERROR;
    // int decimate = q->decimate;
    bzero(q, sizeof(srsran_ue_sync_t));
    q->decimate                     = decimate;
    q->mode                         = mode;
    q->stream_a                     = stream_handler_a;
    q->stream_b                     = stream_handler_b;
    q->stream                       = stream_handler_a;
    q->recv_callback_multi          = recv_callback_multi;
...
static int find_peak_ok(srsran_ue_sync_t* q, cf_t* input_buffer[SRSRAN_MAX_CHANNELS])
{
  if (srsran_sync_sss_detected(&q->sfind)) {
    ...
    if (q->recv_callback(q->stream_a, input_buffer, q->peak_idx, &q->last_timestamp) < 0) {
      return SRSRAN_ERROR;
    }
    return 0;
  }
  ...
  if (q->frame_find_cnt >= q->nof_avg_find_frames || q->peak_idx < 2 * q->fft_size) {
    ...
    if (q->recv_callback(q->stream_a, input_buffer, read_len, &q->last_timestamp) < 0) {
      return SRSRAN_ERROR;
    }
...
static int find_peak_ok_multi_usrp(srsran_ue_sync_t* q, cf_t* input_buffer_a[SRSRAN_MAX_CHANNELS], cf_t* input_buffer_b[SRSRAN_MAX_CHANNELS])
{
  if (srsran_sync_sss_detected(&q->sfind)) {
    /* Get the subframe index (0 or 5) */
    q->sf_idx = (srsran_sync_get_sf_idx(&q->sfind) + q->nof_recv_sf) % 10;
    ...
    INFO("Found peak at %d, No space for SSS. Realigning frame, reading %d samples", q->peak_idx, q->peak_idx);
    if (q->recv_callback_multi(q->stream_a, q->stream_b, input_buffer_a, input_buffer_b, q->peak_idx, &q->last_timestamp) < 0) {
      return SRSRAN_ERROR;
    }
    return 0;
  }
  ...
  if (q->frame_find_cnt >= q->nof_avg_find_frames || q->peak_idx < 2 * q->fft_size) {
    ...
    uint32_t read_len = q->peak_idx + PSS_OFFSET;
    INFO("Realigning frame, reading %d samples", read_len);
    if (q->recv_callback_multi(q->stream_a, q->stream_b, input_buffer_a, input_buffer_b, read_len, &q->last_timestamp) < 0) {
      return SRSRAN_ERROR;
    }
...
static int track_peak_ok(srsran_ue_sync_t* q, uint32_t track_idx)
{
  ...
  if (q->next_rf_sample_offset > 0 && q->next_rf_sample_offset < MAX_TIME_OFFSET) {
    ...
    if (q->recv_callback(q->stream_a, dummy_offset_buffer_a, (uint32_t)q->next_rf_sample_offset, NULL) < 0) {
...
static int track_peak_ok_multi_usrp(srsran_ue_sync_t* q, uint32_t track_idx)
{
  // Get sampling time offset
  q->last_sample_offset = ((int)track_idx - (int)q->strack.max_offset / 2 - (int)q->strack.fft_size);
  ...
  if (q->next_rf_sample_offset > 0 && q->next_rf_sample_offset < MAX_TIME_OFFSET) {
    DEBUG("Positive time offset %d samples.", q->next_rf_sample_offset);
    if (q->recv_callback_multi(q->stream_a, q->stream_b, dummy_offset_buffer_a, dummy_offset_buffer_b, (uint32_t)q->next_rf_sample_offset, NULL) < 0) {
      ERROR("Error receiving from USRP");
      return SRSRAN_ERROR;
    }
    q->next_rf_sample_offset = 0;
...
static int receive_samples(srsran_ue_sync_t* q, cf_t* input_buffer[SRSRAN_MAX_CHANNELS], const uint32_t max_num_samples)
{
  ...
  if (q->recv_callback(q->stream_a, ptr, q->frame_len - q->next_rf_sample_offset, &q->last_timestamp) < 0) {
    return SRSRAN_ERROR;
  }
...
static int receive_samples_multi_usrp(srsran_ue_sync_t* q, 
                                      cf_t* input_buffer_a[SRSRAN_MAX_CHANNELS], 
                                      cf_t* input_buffer_b[SRSRAN_MAX_CHANNELS], 
                                      const uint32_t max_num_samples)
{
  ...
  ///< Get N subframes from the USRP getting more samples and keeping the previous samples, if any
  cf_t* ptr_a[SRSRAN_MAX_CHANNELS] = {NULL};
  cf_t* ptr_b[SRSRAN_MAX_CHANNELS] = {NULL};
  for (int i = 0; i < q->nof_rx_antennas; i++) {
    ptr_a[i] = &input_buffer_a[i][q->next_rf_sample_offset];
  }
  for (int i = 0; i < q->nof_rx_antennas; i++) {
    ptr_b[i] = &input_buffer_b[i][q->next_rf_sample_offset];
  }
  if (q->recv_callback_multi(q->stream_a, q->stream_b, ptr_a, ptr_b, q->frame_len - q->next_rf_sample_offset, &q->last_timestamp) < 0) {
    return SRSRAN_ERROR;
  }
...
int srsran_ue_sync_zerocopy_multi_usrp(srsran_ue_sync_t* q,
                                       cf_t*             input_buffer_a[SRSRAN_MAX_CHANNELS],
                                       cf_t*             input_buffer_b[SRSRAN_MAX_CHANNELS],
                                       const uint32_t    max_num_samples)
{
  ...
  if (q != NULL && input_buffer_a != NULL) {
    if (q->file_mode) {
      int n = srsran_filesource_read_multi(&q->file_source, (void**)input_buffer_a, q->sf_len, q->nof_rx_antennas);
      if (n < 0) {
        ERROR("Error reading input file");
        return SRSRAN_ERROR;
      }
      if (n == 0) {
        if (q->file_wrap_enable) {
          srsran_filesource_seek(&q->file_source, 0);
          q->sf_idx = 9;
          n = srsran_filesource_read_multi(&q->file_source, (void**)input_buffer_a, q->sf_len, q->nof_rx_antennas);
          if (n < 0) {
            ERROR("Error reading input file");
            return SRSRAN_ERROR;
          }
      ...
      if (q->cfo_correct_enable_track) {
        for (int i = 0; i < q->nof_rx_antennas; i++) {
          srsran_cfo_correct(&q->file_cfo_correct, input_buffer_a[i], input_buffer_a[i], q->file_cfo / 15000 / q->fft_size);
        }
      }
      q->sf_idx++;
      if (q->sf_idx == 10) {
      ...
      if (receive_samples_multi_usrp(q, input_buffer_a, input_buffer_b, max_num_samples)) {
        ERROR("Error receiving samples");
        return SRSRAN_ERROR;
      }
      ...
      switch (q->state) {
        case SF_FIND:
          ...
          if (q->cfo_correct_enable_find) {
            for (int i = 0; i < q->nof_rx_antennas; i++) {
              if (input_buffer_a[i]) {
                srsran_cfo_correct(
                    &q->strack.cfo_corr_frame, input_buffer_a[i], input_buffer_a[i], -q->cfo_current_value / q->fft_size);
              }
              if (input_buffer_b[i]) {
                srsran_cfo_correct(
                    &q->strack.cfo_corr_frame, input_buffer_b[i], input_buffer_b[i], -q->cfo_current_value / q->fft_size);
              }
            }
          }
          ...
          // Run mode-specific find operation
          if (q->mode == SYNC_MODE_PSS) {
            ret = srsran_ue_sync_run_find_pss_mode_multi_usrp(q, input_buffer_a, input_buffer_b);
          } else if (q->mode == SYNC_MODE_GNSS) {
            ret = srsran_ue_sync_run_find_gnss_mode(q, input_buffer_a, max_num_samples);
          }

          if (q->do_agc) {
            srsran_agc_process(&q->agc, input_buffer_a[0], q->sf_len);
          }
        ...
        case SF_TRACK:
          ...
          if (q->mode == SYNC_MODE_PSS) {
            srsran_ue_sync_run_track_pss_mode_multi_usrp(q, input_buffer_a, input_buffer_b);
          } else {
            srsran_ue_sync_run_track_gnss_mode(q, input_buffer_a);
          }
          break;
        default:
          ERROR("Unknown sync state %d", q->state);
...
int srsran_ue_sync_run_find_pss_mode(srsran_ue_sync_t* q, cf_t* input_buffer[SRSRAN_MAX_CHANNELS])
{
  ...
  switch (n) {
    ...
    case SRSRAN_SYNC_FOUND_NOSPACE:
      ...
      q->recv_callback(q->stream_a,dummy_offset_buffer_a, q->frame_len / 2, NULL);
      srsran_sync_reset(&q->sfind);
      ret = SRSRAN_SUCCESS;
      break;
    default:
...
int srsran_ue_sync_run_find_pss_mode_multi_usrp(srsran_ue_sync_t* q, cf_t* input_buffer_a[SRSRAN_MAX_CHANNELS], cf_t* input_buffer_b[SRSRAN_MAX_CHANNELS])
{
  int ret = SRSRAN_ERROR;
  int n   = srsran_sync_find(&q->sfind, input_buffer_a[0], 0, &q->peak_idx);
  ...
  switch (n) {
    case SRSRAN_SYNC_ERROR:
      ...
      return ret;
    case SRSRAN_SYNC_FOUND:
      ret = find_peak_ok_multi_usrp(q, input_buffer_a, input_buffer_b);
      break;
    case SRSRAN_SYNC_FOUND_NOSPACE:
      /* If a peak was found but there is not enough space for SSS/CP detection, discard a few samples */
      INFO("No space for SSS/CP detection. Realigning frame...");
      q->recv_callback_multi(q->stream_a, q->stream_b, dummy_offset_buffer_a, dummy_offset_buffer_b, q->frame_len / 2, NULL);
      srsran_sync_reset(&q->sfind);
      ret = SRSRAN_SUCCESS;
      break;
    default:
...
int srsran_ue_sync_run_track_pss_mode_multi_usrp(srsran_ue_sync_t* q, cf_t* input_buffer_a[SRSRAN_MAX_CHANNELS], cf_t* input_buffer_b[SRSRAN_MAX_CHANNELS])
{
  int      ret       = SRSRAN_ERROR;
  uint32_t track_idx = 0;
  /* Every SF idx 0 and 5, find peak around known position q->peak_idx */
  if ((q->sfind.frame_type == SRSRAN_FDD && (q->sf_idx == 0 || q->sf_idx == 5)) ||
    ...
    int n = srsran_sync_find(&q->strack, input_buffer_a[0], pss_idx, &track_idx);
    switch (n) {
      case SRSRAN_SYNC_ERROR:
        ERROR("Error tracking correlation peak");
        return SRSRAN_ERROR;
      case SRSRAN_SYNC_FOUND:
        ret = track_peak_ok_multi_usrp(q, track_idx);
        break;
      case SRSRAN_SYNC_FOUND_NOSPACE:
        // It's very very unlikely that we fall here because this event should happen at FIND phase only
        ret      = 0;
                                      ...
                                      const uint32_t    max_num_samples)
{
  ...
  while (align_len > q->sf_len) {
    ...
    q->recv_callback(q->stream, dummy_offset_buffer_a, actual_rx_len, &q->last_timestamp);
