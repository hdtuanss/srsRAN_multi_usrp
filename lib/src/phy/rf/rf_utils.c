...
int rf_cell_search_multi_usrp(srsran_rf_t*       rf,
                              uint32_t           nof_rx_channels,
                              cell_search_cfg_t* config,
                              int                force_N_id_2,
                              srsran_cell_t*     cell,
                              float*             cfo)
{
  ...
  uint32_t max_peak_cell = 0;
  if (force_N_id_2 >= 0) {
    ret           = srsran_ue_cellsearch_scan_N_id_2_multi_usrp(&cs, force_N_id_2, &found_cells[force_N_id_2]);
    max_peak_cell = force_N_id_2;
  } else {
    ret = srsran_ue_cellsearch_scan_multi_usrp(&cs, found_cells, &max_peak_cell);
  }
...
int rf_search_and_decode_mib_multi_usrp(srsran_rf_t*       rf,
                                      uint32_t           nof_rx_channels,
                                      cell_search_cfg_t* config,
                                      int                force_N_id_2,
                                      srsran_cell_t*     cell,
                                      float*             cfo)
{
  ...
  printf("Searching for cell...\n");
  ret = rf_cell_search_multi_usrp(rf, nof_rx_channels, config, force_N_id_2, cell, cfo);
  if (ret > 0) {
    printf("Decoding PBCH for cell %d (N_id_2=%d)\n", cell->id, cell->id % 3);
    ret = rf_mib_decoder(rf, nof_rx_channels, config, cell, cfo);
    if (ret < 0) {
